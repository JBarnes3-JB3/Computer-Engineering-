...
void button_init() {
	...
	///////////// Notes for later, maybe
    //#define GPIO_PORTE_DATA_R       (*((volatile uint32_t *)0x400243FC))
	//GPIO_PORTE_DATA_R = 0x400F.E00 | 0b0001_0000;
...
//  Vector  Interrupt#   vector address    description
//  20      4           0x0000.0050       GPIO PORTE

//Reg 4    0-31      Set enable (EN0)   offset 0x100
//Reg 5    32-63     Set enable (EN1)   offset 0x104
//Reg 6    64-95     Set enable (EN2)   offset 0x108
//Reg 7    96-127    Set enable (EN3)   offset 0x10c
...
void init_button_interrupts() {
    ...
    // TODOne?: Complete code below

    // 1) Mask the bits for pins 0-3  interrupt mask
    GPIO_PORTE_IM_R &= 0b000;

    // 2) Set pins 0-3 to use edge sensing
    GPIO_PORTE_IS_R &= 0b000;
...
uint8_t button_getButton() {
	...
	// TODOne: Write code below -- Return the left must button position pressed
	
	// INSERT CODE HERE!


    //0 is pushed
    if((GPIO_PORTE_DATA_R & 0b1000) == 0b0000){  //S4 pushed
        return 4;
    }
    else if((GPIO_PORTE_DATA_R & 0b0100) == 0b0000){  //S3 pushed
        return 3;
    }
    else if((GPIO_PORTE_DATA_R & 0b0010) == 0b0000){  //S2 pushed
        return 2;
    }
    else if((GPIO_PORTE_DATA_R & 0b0001) == 0b0000){  //S1 pushed
        return 1;
    }
    else{
        return 0;
