...
public class OrderedStateList 
{
	 ...
	 */
	  ...
	  public void addState(State s)
	  {
		...
		while (curr != head && compareStates(s, curr) > 0) {
            curr = curr.next;
        }

        s.next = curr;
        s.previous = curr.previous;
        curr.previous.next = s;
        curr.previous = s;
	  ...
	  {
		State found = findState(s);
        if (found == null)
            throw new IllegalStateException("State not found in list.");
