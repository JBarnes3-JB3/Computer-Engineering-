...
public class MonteCarloTreeSearch extends AdversarialSearch {
    ...
    private void backPropagation(MCNode<CheckersData> node, double result) {
        // Track whose perspective we're at
        boolean isBlackPerspective = true;
        
        while (node != null) {
            node.visits++;
            
            // For draws (0.5), both players get 0.5
            // For wins/losses, flip the perspective
            if (result == 0.5) {
                node.wins += 0.5;
            } else {
                node.wins += isBlackPerspective ? result : (1.0 - result);
            }
